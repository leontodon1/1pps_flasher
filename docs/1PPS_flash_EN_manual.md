...
It has been shown that a simple idea can be used to create a cost-effective method for accurate time stamps in astronomical videos with very dark areas in the picture in the range of less than a millisecond for any optics. The process of building your own device has been shown in detail \[1\]. The source code is available \[1\]. The author provides support with problems relating to the procurement of parts (suppliers) and has several devices for his own use, which can be loaned out for testing purposes.
